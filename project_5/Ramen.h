...
public:
    ...
    Ramen(int number, float rating, std::string brand, std::string variety, std::string style, std::string country) {
        this->number = number;
        this->rating = rating;
        this->brand = brand;
...
bool inline getRamenData(std::string filename, std::vector<Ramen> &ramenlist) {
    ...
    int number;
    float rating;
    std::string brand, variety, style, country, newLine, junk;
    char comma, quote;
    newLine = "";
